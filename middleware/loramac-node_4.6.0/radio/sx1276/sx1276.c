...
void SX1276WriteBuffer( uint32_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    Gpio_clear( SX1276.Spi.Nss );

    //SpiInOut( &SX1276.Spi, addr | 0x80 );
    address = (uint8_t)addr | 0x80;
    Spi_write( SX1276.Spi.SpiId, &address, 100 );
    for( i = 0; i < size; i++ )
    {
        //SpiInOut( &SX1276.Spi, buffer[i] );
        Spi_write( SX1276.Spi.SpiId, &buffer[i], 100 );
    }
...
//    SpiInOut( &SX1276.Spi, addr & 0x7F );
    address = (uint8_t)addr & 0x7F;
    Spi_write( SX1276.Spi.SpiId, &address, 100 );

    for( i = 0; i < size; i++ )
    {
//        buffer[i] = SpiInOut( &SX1276.Spi, 0 );
        Spi_read( SX1276.Spi.SpiId, &buffer[i], 100 );
    }
